...
class Admin extends CI_Controller
{
    ...
    function settings($param1 = "", $param2 = "", $param3 = "")
    {
        ...
        if ($param1 == "system_image") {
            ...
            if ($_FILES['light_logo']) {
                $target_dir = "uploads/system/";
                $target_file = $target_dir . basename($_FILES["light_logo"]["name"]);
                $imageFileType = strtolower(pathinfo($target_file, PATHINFO_EXTENSION));
                $name = time() . '.' . $imageFileType;
                if (move_uploaded_file($_FILES['light_logo']['tmp_name'], $target_dir . $name)) {
                    $uploadSystemImage = $this->crud_model->update_light_logo($name);
                    if ($uploadSystemImage) {
        ...
        if ($param1 == "favicon") {
            ...
            if ($_FILES['favicon']) {
                ...
                $imageFileType = strtolower(pathinfo($target_file, PATHINFO_EXTENSION));
                $name = time() . '.' . $imageFileType;
                if (move_uploaded_file($_FILES['favicon']['tmp_name'], $target_dir . $name)) {
                    $uploadSystemImage = $this->crud_model->update_favicon($name);
                    if ($uploadSystemImage) {
        ...
        if ($param1 == "banner_image") {
            if ($_FILES['banner_image']) {
                ...
                $imageFileType = strtolower(pathinfo($target_file, PATHINFO_EXTENSION));
                $name = time() . '.' . $imageFileType;
                if (move_uploaded_file($_FILES['banner_image']['tmp_name'], $target_dir . $name)) {
                    $uploadSystemImage = $this->crud_model->update_frontend_banner($name);
                    if ($uploadSystemImage) {
